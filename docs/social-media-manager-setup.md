...
| `SOCIAL_DEFAULT_VISUAL_MODE` | `FULL_AI_GRAPHIC` | Eligible new creatives are complete AI-native graphics; protected Story/product/affiliate formats resolve to verified exact overlay |
...
| `SOCIAL_BRAND_LOGO_PATH` | Optional | Explicit local logo path; otherwise the renderer checks existing Pink Paisa logo candidates |
| `PUBLIC_MEDIA_BASE_URL` | Required for publishable output | Determines the public URL recorded on every rendered asset |

The default for eligible new creatives is `FULL_AI_GRAPHIC` contract version 2: OpenAI renders the complete artwork, exact approved on-image copy and Pink Paisa identity, independent vision validates the complete poster, and Sharp performs resize/encoding-only normalization. The final is a byte passthrough of the normalized image with `overlay.method=none` and `pixel_overlay_applied=false`. Stories and authentic product/affiliate creatives still resolve to `AI_VISUAL_WITH_EXACT_OVERLAY`, where OpenAI supplies the validated text-free artwork/background and guarded Sharp/SVG composition supplies exact required copy or authentic catalogue pixels. Provider originals and final assets remain retained with checksums and provenance. Historical `FULL_AI_GRAPHIC` v1 branded-finish assets remain readable. A missing or invalid original visual fails generation; it does not activate a brand-template background. Template-only Social Media Manager output, including the legacy `MANUAL_TEMPLATE` request mode, is rejected.
